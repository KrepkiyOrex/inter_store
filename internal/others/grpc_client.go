...
func TestInventoryHandler(w http.ResponseWriter, r *http.Request) {
	...
	/* 
		...
		Переменные уже сможешь подставить сразу сюда, за место product123 и warehouse456 и прочее, для декремента.
		сама функция будет срабатывать, только после нажатия кнопки "оформить заказ", засунешь в ту функ.

		с GetInventory как получить тут инфу о наличии на складе? у меня пока так работает, что отправляет 
		инфу на сервак и там все завершается, но нету обратного ответа для вывода на фронт для карты.

		*кстати по поводу соедниения, как будет выглядеть код для интернет соединеня, на случай если будет
		не на хосте у меня а в инете где нить на хостинг?
	*/
